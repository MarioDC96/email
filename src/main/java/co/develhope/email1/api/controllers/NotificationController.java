...
import co.develhope.email1.services.EmailService;
    ...
    public ResponseEntity sendNotification(@RequestBody NotificationDTO payload){
        try{
            ...
            if (studentToNotify == null)
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Studente non trovato");
            emailService.sendTo(studentToNotify.getEmail(), payload.getTitle(), payload.getText());
            return ResponseEntity.status(HttpStatus.OK).build();
        }catch(Exception e){
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("server rotto");
        }
    }
}
